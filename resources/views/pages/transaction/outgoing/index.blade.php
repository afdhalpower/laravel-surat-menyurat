...
@section('content')
    <x-breadcrumb
        ...
        <a href="{{ route('transaction.outgoing.pdf', request()->query()) }}" target="_blank" class="btn btn-success">{{ __('menu.general.export_pdf') }}</a>
        <a href="{{ route('transaction.outgoing.excel', request()->query()) }}" target="_blank" class="btn btn-info">{{ __('menu.general.export_excel') }}</a>
    </x-breadcrumb>
